...
# Give some blocks in the first hotbar slot - CHANGED to stone-block
...
def create_hotbar():
    ...
    for i in range(9):
        ...
        text = Text(parent=hotbar_parent, 
                    text=str(inventory[hotbar_index]["count"]) if inventory[hotbar_index]["item"]!="air" else "",
                    position=(x+0.03,-0.48), scale=0.9, color=color.white, z=-0.2)
        
        item = inventory[hotbar_index]["item"]
        if item != "air":
...
def update_hotbar_selector():
    ...
    for i, (bg, text, icon) in enumerate(hotbar_slots):
        ...
        count = inventory[hotbar_index]["count"]
        text.text = str(count) if inventory[hotbar_index]["item"]!="air" and count > 0 else ""
...
def inventory_click(slot_index):
    ...
    if selected_inventory_slot is None:
        if inventory[slot_index]["item"] != "air":
        ...
        if slot_index != selected_inventory_slot:
            inventory[slot_index], inventory[selected_inventory_slot] = inventory[selected_inventory_slot], inventory[slot_index]
...
def update_held_item_ui():
    if selected_inventory_slot is not None:
        item = inventory[selected_inventory_slot]["item"]
        count = inventory[selected_inventory_slot]["count"]
...
# -----------------------------
def input(key):
    global chat_open, chat_buffer, selected_slot
    ...
    if key=="right mouse down":
        ...
        slot = inventory[hotbar_index]
        
        if slot["item"]!="air" and slot["count"]>0:
            hit = get_look_ray()
...
def update_held_item_model():
    ...
    # No item → hide model
    if item == "air" or inventory[hotbar_index]["count"] == 0:
