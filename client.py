...
class DroppedItem(Entity):
    def __init__(self, position, item_type, count=1):
        try:
            tex = load_texture(item_type)
    ...
    def update(self):
        # Bob up and down
        self.bob_offset += time.dt * self.bob_speed
        self.y = self.initial_y + math.sin(self.bob_offset) * 0.1
        # Rotate slowly
        self.rotation_y += time.dt * 50

dropped_items = []

def drop_item(position, item_type, count=1):
    """Drop an item at the specified position"""
    dropped = DroppedItem(position, item_type, count)
    dropped_items.append(dropped)
    return dropped

def check_item_pickup():
    """Check if player is near any dropped items and pick them up"""
    global dropped_items
    
    for dropped in dropped_items[:]:  # Copy list to iterate safely
        distance = (dropped.position - player.position).length()
        if distance < 2:  # Pickup radius
            # Add to inventory
            current_inv = inventory  # Always add to survival inventory
            
            # Try to stack first
            added = False
            for slot in current_inv:
                if slot["item"] == dropped.item_type:
                    slot["count"] += dropped.count
                    added = True
                    break
            
            # If no stack found, use empty slot
            if not added:
                for slot in current_inv:
                    if slot["item"] == "air" or slot["count"] == 0:
                        slot["item"] = dropped.item_type
                        slot["count"] = dropped.count
                        added = True
                        break
...
def create_hotbar():
    ...
    for i in range(9):
        ...
        bg = Entity(parent=hotbar_parent, model='quad', color=color.rgba(0,0,0,150), 
        ...
        if gamemode == 0:  # Survival - show count
            count_text = str(current_inv[hotbar_index]["count"]) if current_inv[hotbar_index]["item"]!="air" and current_inv[hotbar_index]["count"] > 0 else ""
        ...
        if item != "air" and (gamemode == 1 or current_inv[hotbar_index]["count"] > 0):
...
def update_hotbar_selector():
    ...
    for i, slot_ui in enumerate(hotbar_slots):
        if len(slot_ui) < 3:
            continue
        bg, text, icon = slot_ui[0], slot_ui[1], slot_ui[2] if len(slot_ui) > 2 else None
...
for row in range(rows):
    for col in range(cols):
        ...
        bg = Entity(parent=inventory_parent, model='quad', color=color.rgba(0,0,0,150), 
...
# Create DELETE slot (X button) - positioned in top right of inventory
delete_slot_x = start_x + (cols-0.5)*(slot_size + spacing)
delete_slot_y = start_y + (slot_size + spacing) * 1.5

delete_bg = Entity(parent=inventory_parent, model='quad', color=color.rgba(150,0,0,150), 
                   scale=(slot_size,slot_size), position=(delete_slot_x, delete_slot_y))
delete_border = Entity(parent=inventory_parent, model='wireframe_quad', color=color.red, 
                       scale=(slot_size+0.005,slot_size+0.005), position=(delete_slot_x, delete_slot_y), z=-0.1)
delete_text = Text(parent=inventory_parent, text="X", position=(delete_slot_x-0.02, delete_slot_y-0.015), 
                   scale=2, color=color.white, z=-0.3)

delete_slot_info = (delete_bg, delete_border, delete_text, None)
...
def update_inventory_ui():
    ...
    for i, slot_ui in enumerate(inventory_slots_ui):
        if len(slot_ui) < 3:
            continue
        bg, border, text = slot_ui[0], slot_ui[1], slot_ui[2]
        icon = slot_ui[3] if len(slot_ui) > 3 else None
...
def inventory_click(slot_index):
    ...
    # Check if clicking the delete slot
    if slot_index == -1:
        if selected_inventory_slot is not None:
            # Delete the item
            current_inv = creative_inventory if gamemode == 1 else inventory
            current_inv[selected_inventory_slot]["item"] = "air"
            current_inv[selected_inventory_slot]["count"] = 0
            
            # Remove highlight
            if selected_inventory_slot < len(inventory_slots_ui):
                bg, border, text = inventory_slots_ui[selected_inventory_slot][:3]
    ...
    if selected_inventory_slot is None:
        if current_inv[slot_index]["item"] != "air":
            ...
            # highlight the slot
            bg, border, text = inventory_slots_ui[slot_index][:3]
...
def inventory_input():
    ...
    if current_mouse_state and not last_mouse_state:
        # Check if clicking delete slot
        delete_bg = delete_slot_info[0]
        x_min = delete_bg.x - delete_bg.scale_x/2
        x_max = delete_bg.x + delete_bg.scale_x/2
        y_min = delete_bg.y - delete_bg.scale_y/2
        y_max = delete_bg.y + delete_bg.scale_y/2
        if x_min <= mouse.x <= x_max and y_min <= mouse.y <= y_max:
            inventory_click(-1)  # -1 = delete slot
            last_mouse_state = current_mouse_state
            return
        
        # Mouse was just pressed, process the click on regular slots
        for i, slot_ui in enumerate(inventory_slots_ui):
            if len(slot_ui) < 1:
                continue
            bg = slot_ui[0]
...
def input(key):
    ...
    # Q to drop currently held item
    if key == "q" and not inventory_open and not chat_open:
        hotbar_index = 27 + selected_slot
        current_inv = creative_inventory if gamemode == 1 else inventory
        slot = current_inv[hotbar_index]
        
        if slot["item"] != "air" and (gamemode == 1 or slot["count"] > 0):
            # Drop position in front of player
            drop_pos = player.position + camera.forward * 2 + Vec3(0, 1, 0)
            drop_item(drop_pos, slot["item"], 1)
            
            # Remove from inventory (only in survival)
            if gamemode == 0:
                slot["count"] -= 1
                if slot["count"] <= 0:
                    slot["item"] = "air"
...
def update():
    ...
    check_item_pickup()  # Check for item pickups
